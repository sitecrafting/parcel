...
export default class PackagerRunner {
  ...
  async package(internalBundle: InternalBundle): Promise<Blob> {
    ...
    // Inject the dependency-to-relative-path mapping into all the bundle's
    // assets so that substitution can occur when the asset's output is generated.
    internalBundle.assetGraph.traverseAssets(asset => {
      asset.depToBundlePath = depToBundlePath;
    });
